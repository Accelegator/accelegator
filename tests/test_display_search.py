...
def test_display_search_with_latest_timestamp():
    list = [(("timestamp", True), "email", "field", "response")]
    result = display_search(list)
    num_of_spaces = 66
    assert repr(result) == repr("email" + ' ' * num_of_spaces + "\x1b[7mtimestamp\x1b[0m\nfield\nresponse\n\n")


def test_display_search_with_non_latest_timestamp():
    list = [(("timestamp", False), "email", "field", "response")]
    result = display_search(list)
    num_of_spaces = 66
    assert repr(result) == repr("email" + ' ' * num_of_spaces + "timestamp\nfield\nresponse\n\n")
